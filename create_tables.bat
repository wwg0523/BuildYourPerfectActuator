...
echo [1/6] leaderboard_entries 테이블 생성 중...
docker exec postgres psql -U %DB_USER% -d %DB_NAME% -c "CREATE TABLE IF NOT EXISTS leaderboard_entries (id UUID PRIMARY KEY, user_id UUID, player_name TEXT NOT NULL, company TEXT, score INT NOT NULL, completion_time BIGINT NOT NULL, time_bonus INT DEFAULT 0, final_score INT NOT NULL, played_at TIMESTAMPTZ DEFAULT NOW());"
if %errorlevel% equ 0 (echo ✓ leaderboard_entries 생성 완료) else (echo ✗ 오류 발생)

echo.
echo [2/6] 인덱스 생성 중...
docker exec postgres psql -U %DB_USER% -d %DB_NAME% -c "CREATE INDEX IF NOT EXISTS idx_leaderboard_entries_score_time ON leaderboard_entries(score DESC, completion_time ASC, played_at ASC); CREATE INDEX IF NOT EXISTS idx_leaderboard_entries_played_at ON leaderboard_entries(played_at DESC);"
if %errorlevel% equ 0 (echo ✓ 인덱스 생성 완료) else (echo ✗ 오류 발생)

echo.
echo [3/6] email_logs 테이블 생성 중...
...
echo.
echo [5/6] api_counter_logs 테이블 생성 중...
...
echo.
echo [6/6] api_counter_logs 인덱스 생성 중...
...
docker exec postgres psql -U %DB_USER% -d %DB_NAME% -c "\dt leaderboard_entries email_logs api_counter_logs"
